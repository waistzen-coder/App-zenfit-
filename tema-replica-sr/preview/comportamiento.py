...
with sync_playwright() as p:
    ...
    print('Animación al hacer scroll')
    pg5=b.new_page(viewport={'width':390,'height':844}); pg5.route('**/fonts.g*/**',lambda r:r.abort())
    pg5.goto('file://'+os.path.join(HERE,'producto.html')); pg5.wait_for_timeout(300)
    hidden=[]
    for i in range(pg5.evaluate('document.querySelectorAll(".sr-sec").length')):
        pg5.evaluate('document.querySelectorAll(".sr-sec")[%d].scrollIntoView()'%i); pg5.wait_for_timeout(1600)
        op=pg5.evaluate('getComputedStyle(document.querySelectorAll(".sr-sec")[%d]).opacity'%i)
        if float(op)<0.99: hidden.append((i,op))
    check('todas las secciones se ven al llegar a ellas', not hidden, hidden)

    print('Código de bienvenida')
    pg4=b.new_page(viewport={'width':390,'height':844}); pg4.route('**/fonts.g*/**',lambda r:r.abort())
    nav4=[]; pg4.route('**/cart/**',lambda r:(nav4.append(r.request.url), r.fulfill(status=200,body='checkout')))
    pg4.goto('file://'+os.path.join(HERE,'producto.html')); pg4.wait_for_timeout(300)
    offer=pg4.locator('.sr-offer')
    check('la oferta no sale nada más entrar', offer.is_hidden())
    pg4.evaluate('window.scrollTo(0, document.documentElement.scrollHeight*0.5)'); pg4.wait_for_timeout(700)
    check('sale al bajar por la página', offer.is_visible())
    pg4.locator('[data-sr-offer-apply]').click(); pg4.wait_for_timeout(1500)
    check('se guarda el código', pg4.evaluate("sessionStorage.getItem('sr_code')")=='RELIEF10')
    check('se cierra y queda la pestaña', offer.is_hidden() and pg4.locator('.sr-offer-tab').is_visible())
    line=pg4.locator('#comprar [data-sr-code-line]')
    check('la caja de compra avisa del código', line.is_visible() and 'RELIEF10 aplicado' in line.inner_text(), line.inner_text())
    pg4.locator('#comprar [data-sr-card]').click(); pg4.wait_for_timeout(300)
    check('con 1 unidad el pago lleva discount=RELIEF10', bool(nav4) and 'discount=RELIEF10' in nav4[-1], nav4[-1:] )
    pg4.goto('file://'+os.path.join(HERE,'producto.html')); pg4.wait_for_timeout(300)
    check('la oferta no vuelve a saltar en la misma visita', pg4.locator('.sr-offer').is_hidden())
    pg4.locator('#comprar .sr-pack').nth(1).click(); pg4.wait_for_timeout(50)
    check('con el pack de 2 avisa de que no se suma', 'no se suma' in pg4.locator('#comprar [data-sr-code-line]').inner_text())
    pg4.locator('#comprar [data-sr-card]').click(); pg4.wait_for_timeout(300)
    check('con el pack de 2 el pago va sin código', 'discount=' not in nav4[-1] and ':2?' in nav4[-1], nav4[-1:])
    pg4.goto('file://'+os.path.join(HERE,'portada.html')); pg4.wait_for_timeout(300)
    pg4.locator('.sr-bar a[href="#oferta"]').first.click(); pg4.wait_for_timeout(500)
    check('la barra superior abre la oferta', pg4.locator('.sr-offer').is_visible())
